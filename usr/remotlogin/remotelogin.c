...
int main(int argc, char *argv[])
{
    ...
    // uint16_t listening_port = atoi(argv[2]); // TODO
    ...
    debug_printf("Remote log in is listening on port %d...\n", listening_port);
    while (1) {
        // listen for messages
        char *msg;
        size_t size;
        ...
        // TODO send message to shell and return output

        // echo them back
        err = aos_udp_socket_send(sock, ip, port, msg, size);
        if (err_is_fail(err)) {
            DEBUG_ERR(err, "failed to send response");
            continue;
        }
    }
