...
#define INVOCATION_HANDLER(func)                                                         \
    static struct sysret func(struct capability *kernel_cap,                             \
...
__attribute__((noreturn)) void sys_syscall(uint64_t a0, uint64_t a1, uint64_t a2,
                                           uint64_t a3, uint64_t a4, uint64_t a5,
                                           uint64_t a6, arch_registers_state_t *context);
                                                  ...
                                                  int argc)
{
    ...
    return sys_dispatcher_properties(to, type, sa->arg4, sa->arg5, sa->arg6, sa->arg7,
                                     weight);
}
...
static struct sysret copy_or_mint(struct capability *root,
                                  struct registers_aarch64_syscall_args *args, bool mint)
{
    /* Retrieve arguments */
    ...
    struct sysret sr = sys_copy_or_mint(root, dest_cspace_cptr, destcn_cptr, dest_slot,
                                        source_croot_ptr, source_cptr, destcn_level,
                                        source_level, param1, param2, mint);
    return sr;
}

static struct sysret handle_mint(struct capability *root, arch_registers_state_t *context,
                                 int argc)
{
    assert(11 == argc);
...
static struct sysret handle_copy(struct capability *root, arch_registers_state_t *context,
                                 int argc)
{
    assert(9 == argc);
...
static struct sysret handle_retype_common(struct capability *root, bool from_monitor,
                                          arch_registers_state_t *context, int argc)
{
    assert(11 == argc);
    ...
    return sys_retype(root, source_croot, source_cptr, offset, type, objsize, count,
                      dest_cspace_cptr, dest_cnode_cptr, dest_cnode_level, dest_slot,
                      from_monitor);
}
                                ...
                                arch_registers_state_t *context, int argc)
{
    ...
    return sys_map(ptable, slot, source_root_cptr, source_cptr, source_level, flags,
                   offset, pte_count, mcn_root, mcn_addr, mcn_level, mapping_slot);
}

static struct sysret handle_unmap(struct capability *ptable,
                                  arch_registers_state_t *context, int argc)
{
    assert(4 == argc);
    ...
    err = caps_lookup_slot(&dcb_current->cspace.cap, mapping_cptr, mapping_bits, &mapping,
...
static struct sysret handle_mapping_destroy(struct capability *to,
                                            arch_registers_state_t *context, int argc)
{
    panic("NYI!");
...
static struct sysret handle_mapping_modify(struct capability *to,
                                           arch_registers_state_t *context, int argc)
{
    assert(6 == argc);
    struct registers_aarch64_syscall_args *sa = &context->syscall_args;
...
INVOCATION_HANDLER(monitor_handle_delete_last)
{
    ...
    return sys_monitor_delete_last(root_caddr, root_level, target_caddr, target_level,
                                   retcn_caddr, retcn_level, retcn_slot);
}
...
INVOCATION_HANDLER(monitor_handle_revoke_mark_tgt)
{
    ...
    return sys_monitor_revoke_mark_tgt(root_caddr, root_vbits, target_caddr, target_vbits);
}
...
static struct sysret monitor_get_core_id(struct capability *to,
                                         arch_registers_state_t *context, int argc)
{
    assert(2 == argc);
...
static struct sysret monitor_get_arch_id(struct capability *to,
                                         arch_registers_state_t *context, int argc)
{
    assert(2 == argc);
...
static struct sysret monitor_handle_register(struct capability *to,
                                             arch_registers_state_t *context, int argc)
{
    assert(3 == argc);
...
INVOCATION_HANDLER(monitor_remote_relations)
{
    ...
    return sys_monitor_remote_relations(root_addr, root_bits, cptr, bits, relations, mask);
}
                                        ...
                                        arch_registers_state_t *context, int argc)
{
    ...
    return SYSRET(caps_create_from_existing(&dcb_current->cspace.cap, cnode_cptr,
                                            cnode_vbits, slot, owner, src));
}
...
INVOCATION_HANDLER(monitor_identify_domains_cap)
{
    ...
    err = caps_lookup_cap(&dcb_current->cspace.cap, root_caddr, root_vbits, &root,
                          CAPRIGHTS_READ);
    if (err_is_fail(err)) {
...
static struct sysret dispatcher_dump_ptables(struct capability *to,
                                             arch_registers_state_t *context, int argc)
{
                                         ...
                                         arch_registers_state_t *context, int argc)
{
    ...
    return SYSRET(caps_create_from_existing(&dcb_current->cspace.cap, cnode_cptr,
                                            cnode_level, slot, my_core_id, &devid));
}
...
static struct sysret handle_invoke(uint64_t a0, uint64_t a1, uint64_t a2, uint64_t a3,
                                   ...
                                   arch_registers_state_t *context)
{
    ...
    r.error = caps_lookup_cap(&dcb_current->cspace.cap, invoke_cptr, invoke_bits, &to,
                              CAPRIGHTS_READ);
    if (err_is_ok(r.error)) {
        ...
        if (ObjType_EndPointLMP == to->type && !(flags & LMP_FLAG_IDENTIFY)) {
            ...
            if (listener->disp) {
                ...
                r.error = lmp_deliver(to, dcb_current, msg_words, length_words, send_cptr,
                                      send_bits, give_away);
                ...
                if ((sync && err_is_ok(r.error))
                    ...
                    if (err_is_fail(r.error)) {
                        ...
                        debug(SUBSYS_DISPATCH, "LMP failed; %.*s yields to %.*s: %u\n",
                              DISP_NAME_LEN, current_disp->name, DISP_NAME_LEN,
                              listener_disp->name, err_code);
                    }
                    ...
                    dcb_current->disabled = dispatcher_is_disabled_ip(handle,
                                                                      context->named.pc);
 ...
 *
 */
void sys_syscall(uint64_t a0, uint64_t a1, uint64_t a2, uint64_t a3, uint64_t a4,
                 uint64_t a5, uint64_t a6, arch_registers_state_t *context)
{
    // XXX
    ...
    if (r.error) {
        debug(SUBSYS_SYSCALL, "syscall failed %016" PRIx64 " => %016" PRIxERRV "\n", a0,
              r.error);
    }
