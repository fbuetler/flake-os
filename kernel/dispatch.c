...
void __attribute__((noreturn)) dispatch(struct dcb *dcb)
{
    ...
    if (dcb_current != dcb) {
#ifdef TRACE_CSWITCH
        trace_event(TRACE_SUBSYS_KERNEL, TRACE_EVENT_KERNEL_CSWITCH,
                                 ...
                                 capaddr_t send_cptr, uint8_t send_level, bool give_away)
{
    ...
    struct lmp_endpoint_kern *recv_ep = (void *)((uint8_t *)recv->disp
                                                 + ep->u.endpointlmp.epoffset);
    ...
    err = caps_lookup_cap(&recv->cspace.cap, recv_ep->recv_cspc, 2, &recv_cspace_cap,
    ...
    // Get the cnode
    struct cte *recv_cnode_cte = caps_locate_slot(get_address(recv_cspace_cap), l1index);
    ...
    struct cte *send_cte;
    err = caps_lookup_slot(&send->cspace.cap, send_cptr, send_level, &send_cte,
                           CAPRIGHTS_READ);
    if (err_is_fail(err)) {
 ...
 * \return Error code
 */
errval_t lmp_deliver_payload(struct capability *ep, struct dcb *send, uintptr_t *payload,
                             size_t payload_len, bool captransfer, bool now)
{
    assert(ep != NULL);
    assert(ep->type == ObjType_EndPointLMP);
    struct dcb *recv = ep->u.endpointlmp.listener;
    ...
    struct dispatcher_shared_generic *recv_disp = get_dispatcher_shared_generic(
        recv->disp);
    debug(SUBSYS_DISPATCH, "LMP %.*s -> %.*s\n", DISP_NAME_LEN,
          send ? send_disp->name : "kernel", DISP_NAME_LEN, recv_disp->name);
 ...
 */
errval_t lmp_deliver(struct capability *ep, struct dcb *send, uintptr_t *payload,
                     size_t len, capaddr_t send_cptr, uint8_t send_level, bool give_away)
{
    bool captransfer;
    assert(ep != NULL);
    assert(ep->type == ObjType_EndPointLMP);
